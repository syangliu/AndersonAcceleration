...
def recording(matrix, v1, v2, v3, v4, v5, dType=None):    
    """
...
def AndersonAcc(obj, x0, m, L, mainLoopMaxItrs, funcEvalMax, gradTol=1e-10, 
                show=True, arg='general', record_txt=None, restart=True):
    iters = 0
    orcl = 0
    # x = copy.deepcopy(x0)  
    x = x0.clone() 
    ...
    flag = 0
    gk_m_norm = gk_norm
    xType = 'GD' # NC implies AA step got rejected
...
#            xType = 'GD'
            ...
            if arg == 'general':
                ...
                alphak = acc.cond_num # record cond(XTX)
                gamma = 1/4/L
                c2 = gamma/m 
                c1 = c2
                nu = 2.5
                ...
                if obj(xn, 'f') < fk - gamma*gk_norm**2 + min(
                        c1*gk_m_norm**(nu), c2*gk_m_norm**2, 1E0): 
                    x = xn
                    xType = 'Acc'
                else:
                    x = gx
